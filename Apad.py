...
def play_music(event=None):
	mixer.music.load("assi ishq.mp3")
	mixer.music.play()
...
view.add_checkbutton(label="Tool Bar", image=toolbar_icon, onvalue=True, offvalue=False, variable=show_toolbar, compound=tk.LEFT, command=hide_toolbar)
view.add_checkbutton(label="Status Bar", image=statusbar_icon, onvalue=True, offvalue=False, variable=show_statusbar, compound=tk.LEFT, command=hide_statusbar)
...
count = 0
for i in color_dict:
	color_theme.add_radiobutton(label=i, image=color_icon_list[count], variable=colorvar, compound=tk.LEFT, command=change_theme)
	count += 1
